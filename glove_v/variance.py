...
def load_variance(
    ...
    """
    if approximation:
        approximation_path = f"{download_dir}/{embedding_name}/ApproximationVariances.safetensors"      
        if not os.path.exists(approximation_path):
            raise FileNotFoundError(file_utils.file_loading_error_message(
                'ApproximationVariances.safetensors',
                download_dir,
                embedding_name
            ))
        ...
        with safe_open(approximation_path, framework="numpy") as f:
            ...
            # Otherwise, it must be an SVD approximation
                ...
                raise KeyError(f"[ERROR No approximation found for word index {word_idx}")

    else:
        complete_path = f"{download_dir}/{embedding_name}/CompleteVariances.safetensors"
        if not os.path.exists(complete_path):
            raise FileNotFoundError(file_utils.file_loading_error_message(
                'CompleteVariances.safetensors',
                download_dir,
                embedding_name
            ))

...
def load_vocab(
    ...
    """
    try:
        with open(f'{download_dir}/{embedding_name}/vocab.txt', 'r') as f:
            words = [x.rstrip().split(' ')[0] for x in f.readlines()]
    except FileNotFoundError as err:
        raise file_utils.file_loading_error_message('vocab.txt', download_dir, embedding_name) from err
...
def load_vectors(
    ...
    download_dir: str = f'{file_utils.get_data_path()}/glove-v',
    format: str = 'numpy'
) -> Union[dict[str, np.ndarray], np.ndarray, KeyedVectors]:
    
    possible_formats = ['numpy', 'gensim', 'dictionary']
    assert format in possible_formats, f"Format should be one of the following: {possible_formats}"

    try:
        with safe_open(f'{download_dir}/{embedding_name}/vectors.safetensors', framework="numpy") as f:
            vectors = f.get_tensor("center_vectors")
    except FileNotFoundError as err:
        raise file_utils.file_loading_error_message('vectors.safetensors', download_dir, embedding_name) from err
